...
比赛 ID 任务首次页面抓取失败后，会在同一轮立即淘汰当前代理并更换代理重试
...
暂停爬取和异常四类爬取状态；缺少详情的比赛只计入顶部的数据质量统计。
这些数据库统计每 10 秒刷新一次。
...
显式传入比赛 ID 表示强制刷新，仅受激活的 `crawl_status` 范围限制，
不受上述时间窗口和 1 分钟节奏限制。详情页仍保留脚本执行和 DOM 读取，
以获取动态比分及状态。
...
成功页面会立即写入，下一轮只重试失败或未执行页面。全部 18 页成功写入后才将比赛标记为
`已完成`；任一页失败或超时都保留原状态等待下一轮。
...
| `暂停爬取` | 旧版收尾流程留下的兼容状态，会重新进入历史收尾 |
| `异常` | 旧版收尾流程留下的兼容状态，会重新进入历史收尾 |

默认只有 `未完成` 会进入普通详情和赔率任务。历史收尾还会自动接管
`暂停爬取` 和 `异常`，使旧版留下的比赛能最终进入 `已完成`。如果需要让普通
任务重新处理其他状态，
可以临时修改：
