...
脚本严格按天执行：抓取当天页面，忽略没有 `sId` 的非比赛占位行，查询
`match_ids` 排除当天已存在记录，完成当天所有写入后再处理前一天。剩余 ID 默认
每轮写入 20 个，同一天的写入轮次之间等待 300 秒（5 分钟）；不同日期之间不
等待。以下两个环境变量可修改批次大小和单日批次间隔：
...
“比赛 + 市场”写为 `baseline`，不发送历史存量。之后新出现的市场先
写为 `pending`；只有 `match_ids.created_at` 晚于基线时间的新比赛会进入此流程，
所以基线前已发现但赔率稍后才补齐的比赛也不会误推。Webhook 明确成功后
改为 `sent`，失败记为 `failed`
并在下轮重试。重试前如比赛已不是“未开始”，记录改为 `expired`
且不再发送。Webhook URL 只从环境变量读取，日志不输出该 URL。

通知任务会自动创建自己的两张状态表，但不会自动安装可选赔率视图。
启用通知前先执行：

```bash
psql "$SIMPLE_CRAWLER_DATABASE_URL" -f SimpleCrawler/sql/create_odds_filter_views.sql
```
